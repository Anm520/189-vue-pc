<template>
    <a-modal :maskClosable="false" :open="model" class="ant-modal-dray" @cancel="handleCancel" width="500px">
        ...
        <a-form ref="formRef" :model="formState">
            <a-form-item label="账号" name="account" :rules="{
                required: true,
                message: '请输入云盘账号',
                trigger: 'change',
            }">
                <a-input v-model:value="formState.account" placeholder="请输入云盘账号" disabled
            ...
            <a-form-item label="密码" name="password" :rules="{
                ...
                <a-input v-model:value="formState.password" placeholder="请输入云盘密码"
...
const handleOk = () => {
    ...
    formRef.value
        .validate()
        .then(async () => {
            API.BatchUpdate({ list: [formState] }).then(res => {
